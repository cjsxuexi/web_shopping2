...
<body>
	...
	<form action="${pageContext.request.contextPath }/sys/servlet/handler/productHandlerServlet" 
		...
		<table border="3" align="center" width="600px;">
			...
			<tr>
				<td>商品名称:<input type="text" name="pname" value="${product.pname }"> <br/></td>
			</tr>
			<tr>
				<td>商品分类:
					<select name="productTypeId" onchange="selectProductTypes(this)">
						...
						<c:forEach items="${productTypes }" var="pt">
							<option value="${pt.id }"
								<c:if test="${pt.id == product.firstLevelId }">
									selected="selected"
								</c:if>
							>${pt.ptname }</option>
						</c:forEach>
					...
					<select name="subProductTypeId" id="subProductTypeId">
						...
						<option value="${product.secondLevelId }" selected="selected">${product.secondLevelName }</option>
					</select>
					<br/>
				</td>
			</tr>
			<tr>
				<td>商品作者:<input type="text" name="pauthor" value="${product.pauthor }"> <br/></td>
			</tr>
			<tr>
				<td>商品价格:<input type="text" name="pprice" value=${product.pprice }> <br/></td>
			</tr>
			<tr>
				<td>商品出版社:<input type="text" name="paddress" value="${product.paddress }"> <br/></td>
			</tr>
			<tr>
				<td>商品图片:
					<input type="file" name="pimageUrl"/>
					<img alt="商品图片" width="50px" height="30px;" src="${pageContext.request.contextPath }/upload/${product.pimageUrl }">
					 <br/>
				</td>
			</tr>
			<tr>
				<td>商品说明:<textarea rows="10" cols="30" name="pdesc">
					${product.pdesc }
					</textarea> <br/>
				</td>
			</tr>
